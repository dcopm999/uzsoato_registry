...
def coverage(c):
    ...
    """
    c.run("coverage run --source soato_registry runtests.py tests")
...
def docs(c):
    ...
    """
    c.run("rm -f docs/soato_registry.rst")
    c.run("rm -f docs/modules.rst")
    c.run("sphinx-apidoc -o docs/ soato")
...
def lint(c):
    ...
    """
    c.run("flake8 soato_registry tests")
...
def release(c, bumpsize=''):
    ...
    import soato_registry
    c.run("python setup.py sdist bdist_wheel")
    c.run("twine upload dist/*")

    c.run('git tag -a {version} -m "New version: {version}"'.format(version=soato_registry.__version__))
    c.run("git push --tags")
